...
{
    if (Utilities::fileIsTIM(file.rawData))
    {
        type = TexDBType::TIM;
        loadTIM();
    }
    else if (Utilities::fileIsRTIM(file.rawData))
    {
        type = TexDBType::RTIM;
        loadRTIM();
    }
...
{
    QDataStream rtimStream(file.rawData);
...
{
    QDataStream timStream(file.rawData);
...
{
    QDataStream outStream(&file.rawData, QIODevice::WriteOnly);
